                    ...
                    <tbody>
                     <%
						...
						<tr>
							...
							<td >
							 <select class="form-control select2"  name="userId" id="userId" onchange=assignSupervisor(this.value,<%=managerUser.getUserId() %>)>  
     						 
     						 <%if(managerUser.getManagerId()<1) { %>
     						 <option value="0" >---Assign---</option>
     						 <%} %>
     						 <%
							int l =0;
							while(l<manager.size()) {
									UserViewBean manager1 = manager.get(l);
							%>
			    		 <option value="<%=manager1.getUserId() %>"
			    		 <%if(managerUser.getManagerId()==manager1.getUserId()) { %>
			    		 selected
			    		 <%} %>
			    		 ><%=manager1.getUserName() %></option>
      			           	<%
            			           						l++;
														}
								   					%>				
